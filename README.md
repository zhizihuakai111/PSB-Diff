...
以下为论文在友谊、塘池、江门等地 UAV 数据上的**代表性定量结果**（完整图表与消融见正式论文）。开源后可将可视化图放在 `assets/` 并在下述位置用 Markdown 引用。

### 秧苗 IoU（%，点监督小区域掩码，数值偏低属正常现象）

| 设置 | UNet | DeepLab V3+ | HRNet | SegFormer |
|------|------|-------------|-------|-----------|
| Group 1（0% 伪标签） | 38.3 | 24.88 | 14.95 | 39.45 |
| Group 4（75% 伪标签） | 29.45 | 27.41 | 12.65 | 32.72 |

### 根部定位 F1（SegFormer，Group 1，距离阈值 r 像素）
